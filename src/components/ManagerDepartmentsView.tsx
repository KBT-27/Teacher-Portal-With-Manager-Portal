...
export const ManagerDepartmentsView: React.FC<ManagerDepartmentsViewProps> = ({
  ...
  const handleDelete = (id: string) => {
    if (!confirm('Are you sure you want to remove this academic department?')) {
      return;
    }
          ...
          >
            <div>
              ...
              <div className="mt-4 p-3 bg-slate-50 rounded-2xl border border-slate-100 space-y-1.5 text-xs">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-slate-400 font-bold uppercase">Subjects Offered</span>
                  <span className="font-semibold text-blue-600">{dept.subjects ? dept.subjects.join(', ') : 'General'}</span>
                </div>
              </div>
            </div>
