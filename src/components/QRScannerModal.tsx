...
export const QRScannerModal: React.FC<QRScannerModalProps> = ({
  ...
  const handleDetectedCode = (codeString: string) => {
    ...
    if (!isValid) {
      if (lastInvalidCodeRef.current !== codeString) {
        ...
        setInvalidQrWarning('Rejected: Invalid or unrecognized QR code. Only the official entrance QR code generated by the Academic Manager / Mentor for today is accepted.');
  ...
  const handleManualLinkSubmit = (e: React.FormEvent) => {
    ...
    if (!isValid) {
      setManualLinkError('Invalid attendance link or code. Only the link shared by your mentor/manager for today is accepted.');
      return;
    }
